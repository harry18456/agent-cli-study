...
| `codebus-core/src/agent/codex_backend.rs:90` | Codex argv 組裝集中在 `CodexBackend::build_command` |
...
| enforcement 層級 | 這是 Claude Code permission layer 行為，不是 OS/kernel-enforced boundary；每次 Claude Code 升級必須重跑 sandbox spike |
...
| default/elevated `:read-only` / `:workspace` | 仍 exit `1`：`windows sandbox failed: spawn setup refresh` |
| explicit `windows.sandbox="elevated"` | 仍 exit `1`：`spawn setup refresh` |
| `windows.sandbox="unelevated"` + built-in `:read-only` / `:workspace` | shell 可啟動，`Get-Location` 成功 |
| `unelevated :workspace` workspace 內讀寫 | 成功 |
| `unelevated :workspace` workspace 外讀取 | 成功，讀到 `OUTSIDE_BOUNDARY_SECRET` |
| `unelevated :workspace` workspace 外寫入 | 失敗，`Access denied`，檔案不存在 |
...
| `multi_agent` 未 disabled | `spawn_agent` 繼承 session `-s`；寫/命令面 bounded，讀面仍同樣 soft-partial |
...
codebus 自己注入的 provider key 也在 child env：Claude Azure profile 注入 `ANTHROPIC_API_KEY`，Codex Azure profile 注入 `CODEBUS_CODEX_AZURE_KEY`。Codex workspace-write shell/subagent 可直接觀察 env；Claude 端是否可外洩取決於可用工具面，但 key 仍存在於 CLI child process environment。

企業結論：agent runner 必須使用乾淨 env allowlist 與 provider key broker。只靠 `CODEBUS_HOME`、`CODEX_HOME` 不能處理 env secret。

### raw_sync 與 live-read 不對稱
...
| 類型 | 行為 | 風險 |
|---|---|---|
| gitignored 檔案 | 不進 raw mirror，也不掃描 PII | 正常 mirror 讀取路徑不暴露；但 Codex Windows `-s` 可越界 live-read，Claude 絕對路徑 `Read` 也可讀 live repo，且該檔從未掃描 |
| root `.env` | `ALWAYS_SKIP_AT_ROOT` 排除 | 同上 |
| UTF-16LE / 非 UTF-8 | 不掃 PII，byte-identical 複製進 raw mirror | secret 可未遮罩進 vault |
| PII scanner pattern | built-in 只含 AWS `AKIA/ASIA`、Anthropic `sk-ant-`、email、IPv4；只有前兩者是 Critical floor | GitHub PAT、GCP key、Slack token、JWT、PEM private key body、DB URL password 等即使 UTF-8 也不會被遮罩，除非企業自加 `patterns_extra` |
| oversized `>5 MiB` | 不進 raw mirror，寫 `_codebus-oversized.md` manifest | 正常 mirror 讀取路徑不暴露；但 Codex/Claude 越界 live-read 仍可讀未掃描的大檔 |
| symlink / NTFS junction | raw mirror walk 未設定 follow-links；junction live-deref 尚未重測 | 惡意 repo 可能放 junction 指到 home secret；Codex Windows `-s` 讀取不擋，屬待驗邊界 |

本機 probe 已重現：`.gitignore` 內 `secrets/` 未進 mirror；UTF-16LE 檔案內的 AWS-like marker byte-identical 進 raw mirror，解碼後仍含 secret。

企業結論：raw mirror 需要非 UTF-8 fail-closed 或 decode/scan；PII scanner pattern 必須由企業擴充；gitignored/oversized skip 需要 manifest/audit；Codex/Claude live-read 破口要用 OS/filesystem boundary 或 allowlist hook 補。

### audit stderr 黑洞

`invoke` 預設把 child stderr drain 到 `io::sink()`；只有設定 `CODEBUS_FORWARD_AGENT_STDERR=1` 才轉發。`sandbox_denial_count` 只從 stdout stream 的 failed tool result 解析 `Access is denied` / `PermissionDenied` 等 marker。

風險：Codex 在 Windows 的 sandbox 診斷、spawn setup refresh、部分 permission denial 可能只出現在 stderr；此時頂層 `codex exec` 仍可能 exit 0，codebus run 會標成 succeeded，`sandbox_denial_count` 也維持 0。

企業結論：目前 audit gate 不能只要求「保存 sandbox denial」，還要先修 codebus stderr capture / structured classification，否則 stderr-only denial 不可觀測。

### vault hook integrity

Claude write-confine 把寫入限制在 vault，但 `.codebus/.claude/settings.json` 本身就在 vault 內。`goal/fix` 這類 workspace toolset 有 `Write/Edit`，若被 prompt injection 誘導，可修改下輪 spawn 使用的 `check-bash` / `check-read` hook 設定。

企業結論：vault 內安全設定檔需要 deny-write、spawn 前重建、hash 驗證，或把 hook policy 移出 agent 可寫範圍。
...
| mirror/live-read model | codebus 正常設計讀 `raw/code` mirror；但 Codex Windows 與 Claude 絕對路徑讀取都可能越過 mirror 讀 live repo |
...
| subagent | Claude `Task` 不在 toolset；Codex `multi_agent` 未 disabled，子 agent 繼承 `-s` 但讀面仍 soft-partial |
| stderr/audit | 兩者共用 invoke；child stderr 預設 sink，stderr-only denial 不會進 `sandbox_denial_count` |
...
9. 不得把 raw mirror PII scan 等同 live repo 讀取保護。
10. 不得把 stdout JSONL audit 等同完整 child stderr audit。
...
| filesystem smoke | workspace 內讀寫、workspace 外讀寫、user home secret、sibling repo、`.git`、`.codebus`、symlink/junction 全部測 |
| network smoke | deny-all、provider allowlist、MCP allowlist、loopback、HTTP/80、HTTPS/443 全部測 |
| raw mirror smoke | gitignored secret、UTF-16/非 UTF-8 secret、oversized file、nested `.git`、PII mask/skip/warn、GitHub/GCP/Slack/JWT/PEM false-negative 全部測 |
| persistence smoke | session/history/cache/memory 不跨任務 |
| audit | 保存 JSONL、exit code、sandbox denial、changed paths；codebus 現況預設 sink child stderr，須先修 stderr capture 才能滿足此 gate |
...
| codebus + Codex native Windows elevated | 需受控 runner 重測；本機 `0.135.0` 與 `0.136.0` elevated/default backend 均未通過 smoke |
...
| Codex actual argv | `codebus-core/src/agent/codex_backend.rs:90-291` |
| Codex azure key env injection without clear | `codebus-core/src/agent/codex_backend.rs:247` |
| Codex `multi_agent` intentionally enabled | `codebus-core/src/agent/codex_backend.rs:122-124` |
| Codex Windows sandbox observations | `D:\side_project\codebus\docs\security.md` §5 |
| child stderr sink / denial count | `codebus-core/src/agent/claude_cli.rs:192-215`、`:256-264` |
| PII scanner built-ins | `codebus-core/src/pii/scanners/regex_basic.rs:22-53` |
| raw mirror gitignore / oversized / non-UTF-8 | `codebus-core/src/vault/raw_sync.rs:84`、`:128`、`:214`、`:272`、`:305`、`:334` |
...
| RegexBasicScanner source inspection | built-in 只有 AWS/Anthropic/email/IPv4；GitHub/GCP/Slack/JWT/PEM 非內建 |
| stderr observability source inspection | child stderr 預設 sink；denial count 只看 stdout tool result |
...
2. 將 provider key 從 child env 移到 broker，或至少禁止 workspace-write shell/subagent 讀 env。
3. 將 Claude `check-read` 從 denylist 改為 vault allowlist，或至少替 `Glob/Grep` 增加同等 path gate。
4. 將 vault 安全設定檔 `.claude/settings.json`、`CLAUDE.md`、`.codex/skills` 從 agent 可寫範圍移出或加 hash/restore。
5. 修 raw_sync：gitignored/oversized secret 要有企業可見報告；非 UTF-8 檔案不得 byte-identical 帶入 raw mirror 而不掃描。
